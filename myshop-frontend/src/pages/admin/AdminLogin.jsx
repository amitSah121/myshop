...
function AdminLogin() {
  ...
  // ✅ Handle Google OAuth redirect (same as Login.jsx)
  useEffect(() => {
    ...
    if (token && userStr) {
      ...
      if (user.isAdmin || user.role === 'admin' || user.email.includes('admin')) {
        ...
        alert(`Welcome to Admin Portal, ${user.name}! 🏢`);
  ...
  const handleGoogleLogin = () => {
    // Redirect to backend Google OAuth with returnUrl parameter
    window.location.href = 'http://localhost:5000/api/auth/google?returnUrl=/admin/login';
  };
